---
título: Personalização visual
descrição: “Vários aspectos do layout visual do Zed podem ser configurados no Editor de Configurações ou no arquivo de configurações.”
---

# Personalização visual

Vários aspectos da aparência do Zed podem ser configurados no Editor de Configurações ({#kb zed::OpenSettings}) ou no seu arquivo de configurações ({#kb zed::OpenSettingsFile}).

Consulte [Todas as configurações](./reference/all-settings.md) para obter informações adicionais e conhecer outras configurações não visuais.

## Temas

Você pode instalar diversos [temas](./themes.md) e [temas de ícones](./icon-themes.md) na forma de extensões executando {#action zed::Extensions} na paleta de comandos.

Você pode visualizar/escolher entre os temas e temas de ícones instalados usando {#action theme_selector::Toggle} ({#kb theme_selector::Toggle}) e {#action icon_theme_selector::Toggle}, que modificarão as seguintes configurações:
...
Se você quiser usar temas diferentes para o modo claro e o modo escuro, que podem ser configurados com:
...
Por padrão, o Zed habilita ligaduras de fontes, que combinam visualmente certos caracteres adjacentes.

Por exemplo, `=>` será exibido como `→` e `!=` será exibido como `≠`.
Trata-se de uma alteração meramente estética, e os personagens individuais permanecem inalterados.
...
<!--
A definir: Configurações relacionadas ao layout centralizado
```json [settings]
    "centered_layout": {
...
### Barra de rolagem do editor e minimapa {#editor-scrollbar}
...
### Sugestões de autocompletamento do editor, trechos de código, ações, diagnósticos {#editor-lsp}
...
### Dicas para inserções no editor
...
O painel do projeto pode ser exibido/ocultado com {#action project_panel::ToggleFocus} ({#kb project_panel::ToggleFocus}) ou com {#action pane::RevealInProjectPanel} ({#kb pane::RevealInProjectPanel}).
...
Consulte a [Documentação do Zed AI](./ai/overview.md) para obter informações sobre outras configurações de IA não visuais.
...
Consulte [Configurações do terminal](./reference/all-settings.md#terminal) para conhecer outras opções de personalização não visuais.
