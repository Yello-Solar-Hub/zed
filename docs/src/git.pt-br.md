...
### Destaque de diferenças no Word
...
### Estilos de visualização de diferenças

O Zed exibe as diferenças em dois modos: **dividido** (comparação lado a lado) ou **unificado** (alterações em linha). A visualização dividida é a padrão.

#### Alterando a visualização de diferenças

Abra o Editor de Configurações ({#kb zed::OpenSettings}) e procure por “estilo de visualização de diferenças”. Selecione **Dividido** ou **Unificado**.

Para alterar a configuração padrão, adicione o seguinte ao seu arquivo `settings.json`:
...
Consulte [Configurando o Zed](./configuring-zed.md) para saber mais sobre o Editor de Configurações.

#### Dividido x unificado

- **Comparar**: Exibe as versões original e modificada lado a lado. Útil para comparar a estrutura do arquivo ou analisar grandes alterações.
- **Unificado**: Mostra as alterações diretamente no texto, incluindo adições e exclusões, em uma única visualização. Útil para se concentrar em alterações específicas em determinadas linhas.

Você pode alternar entre os modos a qualquer momento. Sua preferência se aplica a [Diferenças do projeto](#project-diff), [Histórico do arquivo](#file-history) e [Visualização de diferenças do Stash](#stash-diff-view). Essas visualizações de diferenças funcionam como [multibuffers](./multibuffers.md), permitindo que você edite vários trechos simultaneamente.
...
<!-- Mostrar um conjunto de alterações com o status “staged” por padrão -->
...
### Criação e alternância entre ramificações
...
Ao trabalhar em uma [árvore de trabalho do Git](#git-worktrees), use o seletor de ramificações após alternar para a árvore de trabalho para criar ou fazer o check-out da ramificação que deseja usar ali.
...
## Árvores de trabalho do Git

As árvores de trabalho do Git permitem manter várias cópias do mesmo repositório no disco ao mesmo tempo.
Isso é útil quando você deseja trabalhar em mais de um branch ou tarefa sem precisar salvar temporariamente, recompilar ou alterar os arquivos na sua cópia principal.

Abra o seletor de árvore de trabalho na barra de título, ao lado do seletor de projeto, ou executando {#action git::Worktree}.
No seletor, você pode:

- Crie uma nova árvore de trabalho vinculada a partir do branch atual ou do branch padrão
- Digite um nome para criar uma árvore de trabalho com nome ou deixe que o Zed escolha um automaticamente para você
- Alterar o espaço de trabalho atual para uma árvore de trabalho existente
- Abrir uma árvore de trabalho existente em uma nova janela
- Excluir árvores de trabalho vinculadas que não estejam abertas no momento no projeto

### Gerenciamento da árvore de trabalho

Novas árvores de trabalho são criadas no estado HEAD desanexado.
Depois de mudar para a nova árvore de trabalho, use o seletor de ramificações ao lado do seletor de árvores de trabalho para criar uma nova ramificação ou fazer o check-out de uma ramificação existente que ainda não tenha sido utilizada.
Isso evita que o Zed faça o check-out acidentalmente do mesmo branch em várias árvores de trabalho.

O diretório utilizado para novas árvores de trabalho é controlado pela configuração `git.worktree_directory`.
Por padrão, o Zed cria árvores de trabalho em `../worktrees`, em relação ao diretório de trabalho do repositório.

Consulte [Todas as configurações](./reference/all-settings.md#git-worktree-directory) para ver exemplos.

### Configuração inicial

Para executar etapas de configuração depois que o Zed criar uma árvore de trabalho vinculada, use o [gancho de tarefa `create_worktree`](./tasks.md#hooks).
Para fluxos de trabalho específicos de agentes, consulte [Isolamento da árvore de trabalho](./ai/parallel-agents.md#worktree-isolation).

### Espaços de trabalho com várias raízes

Se o seu projeto contiver vários repositórios Git (ou seja, pastas com várias raízes), o Zed cria uma árvore de trabalho vinculada para cada repositório ao criar uma nova árvore de trabalho a partir do seletor.
As pastas que não estão no Git, mas que fazem parte do mesmo projeto, são incluídas no novo espaço de trabalho tal como estão.
...
### Operações de armazenamento rápido
...
### Visualização de diferenças do Stash
...
Você pode pedir à IA para gerar uma mensagem de commit acessando o editor de mensagens no Git Panel e clicando no ícone de lápis no canto inferior esquerdo, selecionando a ação {#action git::GenerateCommitMessage} ou usando o atalho de teclado {#kb git::GenerateCommitMessage}.

> Observe que é necessário ter um provedor de LLM configurado, seja por meio de suas próprias chaves de API, seja por meio dos modelos de IA hospedados pela Zed.
> Acesse [Guia de Início Rápido da IA](./ai/quick-start.md) para saber como configurar a IA.

Você pode especificar o modelo de sua preferência para essa tarefa adicionando um campo `commit_message_model` às configurações do seu agente.
...
Para adicionar instruções personalizadas de commit para o modelo, use o arquivo global `AGENTS.md`, localizado em `~/.config/zed/AGENTS.md` no macOS e no Linux, e em `%APPDATA%\Zed\AGENTS.md` no Windows.
...
> Antes da versão 1.4.0 do Zed, isso era feito por meio da Biblioteca de Regras, que foi removida.
> Consulte [Migração a partir de regras](./ai/instructions.md#migrating-from-rules) para obter mais informações.
...
## Atalhos de teclado do Diff Hunk
...
> Nem todas as ações possuem atalhos de teclado padrão, mas podem ser configuradas [personalizando seu mapa de teclas](./key-bindings.md#user-keymaps).

## Configuração da CLI do Git
